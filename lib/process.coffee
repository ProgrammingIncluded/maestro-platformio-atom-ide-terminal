...
filteredEnvironment = do ->
  ...
  env.TERM_PROGRAM = 'maestro-platformio-ide-terminal'
  return env
...
module.exports = (pwd, shell, args, env, options={}) ->
  ...
  emitTitle = _.throttle ->
    emit('maestro-platformio-ide-terminal:title', ptyProcess.process)
  , 500, true

  ptyProcess.on 'data', (data) ->
    emit('maestro-platformio-ide-terminal:data', data)
    emitTitle()

  ptyProcess.on 'exit', ->
    emit('maestro-platformio-ide-terminal:exit')
    callback()
  ...
  process.on 'message', ({event, cols, rows, text}={}) ->
    switch event
      ...
      when 'pty' then emit('maestro-platformio-ide-terminal:pty', ptyProcess.pty)
